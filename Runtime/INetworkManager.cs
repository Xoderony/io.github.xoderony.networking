...
namespace Xoderony.Networking
{
    ...
    public interface INetworkManager
    {
        ...
        void SendToOthers(byte messageType, BufferWriter payload, NetworkDelivery delivery = NetworkDelivery.Reliable);

        /// <summary>广播给所有已连接对端并本地投递（本地回显走同一 handler）。</summary>
        void SendToAll(byte messageType, BufferWriter payload, NetworkDelivery delivery = NetworkDelivery.Reliable);

        /// <summary>定向发送给指定对端（需已建立连接）。</summary>
        void SendToPeer(ulong peerId, byte messageType, BufferWriter payload, NetworkDelivery delivery = NetworkDelivery.Reliable);
